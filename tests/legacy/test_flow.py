...
def test_end_to_end(tmp_path):
    ...
    _ = isq.qiskit.check_jobs_status(execute_dataframe, service=service, jobs=jobs)

    _ = partial(isq.qiskit.extract_results_v3, qubit_idx=QUBIT_IDX)
    ...
    _, model_key, gate_optim_key, _ = jax.random.split(master_key, 4)
    ...
    _ = isq.model.default_adaptive_lr_transform(
        PATIENCE=10,
        COOLDOWN=0,
        FACTOR=0.1,
        RTOL=1e-4,
    ...
    model_params, _, history = isq.model.train_model(
        key=model_key,
        model=model,
        optimiser=optimiser,
