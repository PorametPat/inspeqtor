...
def load_model(path: os.PathLike | str, skip_history: bool = False):
    path = str(path)
...
def get_predict_expectation_value(
    ...
    assert isinstance(Wos_params, dict)
...
def MAEF_loss_fn_with_dropout(
    params: VariableDict,
    pulse_parameters: jnp.ndarray,  # noqa: F722
    unitaries: jnp.ndarray,  # noqa: F722
    expectation_values: jnp.ndarray,  # noqa: F722
    training: bool,
...
def create_train_step(
    ...
    loss_fn_with_dropout = (
        MSEE_loss_fn_with_dropout
        if loss_type == LossChoice.MSEE
        else MAEF_loss_fn_with_dropout
    )
    ...
    def train_step(
        ...
        pulse_parameters: jnp.ndarray,  # noqa: F722
        unitaries: jnp.ndarray,  # noqa: F722
        expectations: jnp.ndarray,  # noqa: F722
        dropout_key: jnp.ndarray,
        lr_transformer_state: None,
    ):
