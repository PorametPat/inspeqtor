...
def prepare_experiment(
    ...
    IS_REAL_BACKEND = "fake" not in backend_properties.name
...
def execute_experiment(
    ...
    with Session(
        ...
        sampler = Sampler(mode=session, options=options)  # pyright: ignore
...
def extract_results_v2(result, job_id: str) -> pd.DataFrame:
    ...
    for circuit_idx, res in enumerate(result):
        ...
        num_shots = res.data.c.num_shots  # NOTE: .c because of the name of classical register https://docs.quantum.ibm.com/api/migration-guides/v2-primitives#steps-to-migrate-to-sampler-v2
...
def create_circuit(
    ...
    assert isinstance(qc, QuantumCircuit)  # pyright: ignore
    assert isinstance(crs, list)  # pyright: ignore
    for cr in crs:
    ...
    if add_pulse:
        with pulse.build(backend=backend.backend_instance, name="pulse") as test_pulse:
        ...
        qc.add_calibration("pulse", qubit_indices, test_pulse)
        custom_gate = Gate(name="pulse", num_qubits=num_qubits, params=[])
...
def prepare_parallel_experiment_v2(
    ...
    control_sequences: list[ControlSequence],
...
def extract_results_v3(
    ...
    for circuit_idx, res in enumerate(result):
        ...
        # Get the counts
        counts = bitarray.get_counts()
        num_shots = bitarray.num_shots  # NOTE: .c because of the name of classical register https://docs.quantum.ibm.com/api/migration-guides/v2-primitives#steps-to-migrate-to-sampler-v2
        assert isinstance(counts, dict) and isinstance(num_shots, int)
...
def make_sampler_options(shots: int):
    return SamplerOptions(
        default_shots=shots,  # pyright: ignore
        dynamical_decoupling=DynamicalDecouplingOptions(  # pyright: ignore
            enable=False  # pyright: ignore
        ),
        twirling=TwirlingOptions(  # pyright: ignore
            enable_gates=False,  # pyright: ignore
            enable_measure=False,  # pyright: ignore
        ),
    )
